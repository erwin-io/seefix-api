...
app.get("/", (_req, res) => {
  res.status(200).json({
    service: "seefix-api",
    message: "POST one image field to /api/analyze.",
  });
});
...
app.get("/health", (_req, res) => {
  try {
    ...
    res.status(500).json({ status: "configuration_error", detail: error.message });
  }
});

app.post("/api/analyze", upload.single("image"), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ detail: "An image file is required." });
  }
  ...
  try {
    ...
    const code = knownError ? error.code : "UNEXPECTED_ERROR";
    const status =
      code === "AGENT_TIMEOUT"
        ? 504
    ...
    console.error("SEEFIX agent error", {
      ...
      cause: error instanceof Error ? error.cause?.message : undefined,
    });
...
if (!process.env.VERCEL && isDirectRun) {
  app.listen(port, "127.0.0.1", () => {
    console.log(
      `SEEFIX API running at http://127.0.0.1:${port} (${resolveAgentProvider()} agent)`,
    );
  });
}
